 ...
 */
@JvmName("mapMulti")
fun <T : Any> River.map(type: Class<T>, mapper: RiverMappingReceiver.(T) -> Unit) = this.map(type.kotlin, mapper)
 ...
 */
fun <T : Any> RiverTypeMappers.add(type: Class<T>, mapper: RiverTypeMapper<T>) = this.addMapper(type.kotlin, mapper)
